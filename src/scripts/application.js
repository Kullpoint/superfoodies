...
const addToCartLogic = () => {
    ...
    if (false) {
        addToCartButton.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            const productId = productForm.querySelector('[name="id"]');
            const productCount = addToCartButton.parentNode.parentNode.querySelector("[data-product-qty]");
            const updateObj = {};
            ...
            if (Array.from(document.querySelectorAll("[data-product-item]")).some((product) => product.getAttribute("data-product-item") == productId.value)) {
                for (const product of document.querySelectorAll("[data-product-item]")) {
                    if (product.getAttribute("data-product-item") == productId.value) {
                        ...
                        updateObj[productId.value] = +productCount.value + +quantityInput.value;
                        break;
                    }
                }
            }
            else {
                updateObj[productId.value] = +productCount.value;
            }
